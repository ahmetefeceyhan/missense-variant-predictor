"""
============================================================================
TEKNOFEST 2026 Sağlıkta YZ  —  PAH panel  —  MULTI-FEATURE model comparison
============================================================================

Part 1 (PAH_single_feature_eval.py) showed that, on their own, almost no feature
beats the majority-class floor: only 18 of 351 features score single-feature F1
above 0.9051. This script asks the next question: does COMBINING many features
predict PAH `Label` (0 = benign, 1 = pathogenic) better than any single feature?
...
Selection methods:
  single_f1       reuse PAH_single_feature_results.csv (best single-feature F1
                  per feature, descending). CONTINUITY with Part 1. NOTE: this
                  ranking was measured on the same test fold, so this selector is
                  mildly optimistic vs the train-only selectors below; ranks past
                  ~18 are arbitrary floor-ties.
...
k sweep: 10, 25, 50, 100, 150, 200, 250, and all (=351). k=all is identical for
every method, so it is emitted once as `all_features`.  => 4*7 + 1 = 29 models.
...
Evaluation: the SAME stratified 80/20 holdout (random_state=42) as Part 1.

Run:
    python3 PAH_feature_in_depth/PAH_multi_feature_eval.py
Output:
    PAH_feature_in_depth/PAH_multi_feature_results.csv   (sorted best F1 -> worst)
    PAH_feature_in_depth/PAH_multi_feature_ksweep.png    (if matplotlib present)

CAVEAT: the test fold has only 13 benign rows, so metrics are noisy — treat the
...
# Reuse the Part 1 helpers so preprocessing stays identical.
from PAH_single_feature_eval import (MISSING_CAT, PANEL, SEED, apply_transform,
                                     feature_columns, is_categorical,
                                     load_panel, locate_data_dir, new_xgb)
...
# except single_f1 which reuses the Part 1 CSV (documented caveat).
# ---------------------------------------------------------------------------
def rank_single_f1(names):
    path = os.path.join(HERE, "PAH_single_feature_results.csv")
    r = pd.read_csv(path)
...
def maybe_plot(res):
    ...
    ax.set_title("PAH multi-feature: F1 vs k by selection method")
    ax.legend()
    ax.grid(alpha=0.3)
    out = os.path.join(HERE, "PAH_multi_feature_ksweep.png")
    fig.tight_layout()
...
def main():
    ...
    rankings = {
        "single_f1": rank_single_f1(names),
    ...
    out_csv = os.path.join(HERE, "PAH_multi_feature_results.csv")
    ...
    print("Reference — best SINGLE-feature F1 (Part 1) = 0.9185 (AL_66)")
