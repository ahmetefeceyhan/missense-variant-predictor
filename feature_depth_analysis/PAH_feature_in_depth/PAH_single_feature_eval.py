"""
============================================================================
TEKNOFEST 2026 Sağlıkta YZ  —  PAH panel  —  SINGLE-FEATURE model comparison
============================================================================

Goal: measure the standalone predictive power of every individual feature in
the PAH dataset. For each non-ID feature we train a basic XGBoost classifier
(n_estimators=100, otherwise default — "nothing fancy") that uses ONLY that one
feature to predict Label (0 = benign, 1 = pathogenic), then compare
accuracy / F1 / recall / precision across features.
...
SCOPE: PAH dataset ONLY. No augmentation from MASTER / KANSER / CFTR.
EVAL:  a single stratified 80/20 holdout split (random_state=42), reused for
       every model so the numbers are directly comparable.

Run:
    python3 advisee_models/PAH_single_feature_eval.py
Output:
    advisee_models/PAH_single_feature_results.csv   (sorted best F1 -> worst)
    + a console summary (top-20 table, per-feature best variant).

CAVEAT: the test fold has only ~12 benign rows, so per-feature metrics are
noisy — treat the ranking as relative, not absolute. The `significand`
transform deliberately discards magnitude, so it can break rank order.
"""
import argparse
import glob
import math
...
# ---------------------------------------------------------------------------
def locate_data_dir(explicit=None):
    if explicit:
        return explicit
    here = os.path.dirname(os.path.abspath(__file__))
    for up in range(6):
        base = os.path.abspath(os.path.join(here, *([".."] * up)))
        hits = [h for h in glob.glob(os.path.join(base, "**", "*PAH*.csv"), recursive=True)
                if "universite-veri-seti" in h]
        if hits:
            return os.path.dirname(hits[0])
    raise FileNotFoundError("Could not locate the data folder; pass --data_dir.")
...
# ---------------------------------------------------------------------------
def self_check():
    """Verify the significand example from the spec."""
...
def main():
    ...
    out_csv = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           "PAH_single_feature_results.csv")
